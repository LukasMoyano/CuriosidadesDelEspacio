...
function App() { // Definimos el componente principal de la aplicación
  ...
  const handleChangeQuote = () => { // Definimos la función que se ejecuta cuando se hace clic en el botón de cambio de cita
  ...
  return ( // Retornamos el contenido del componente App
    <main className={`app ${bgImage}`}> {/* Definimos la clase CSS para el fondo de la aplicación utilizando el estado de la imagen de fondo actual */}
      <section className="app__container px-4 py-8 lg:px-0 lg:w-1/2 lg:m-auto"> {/* Definimos la sección principal de la aplicación */}
        <h1 className="text-center text-4xl font-bold text-black">Curiosidades del Espacio</h1> {/* Definimos el título principal de la aplicación */}
        <QuoteBox handleChangeQuote={handleChangeQuote} phrase={quote.phrase} /> {/* Utilizamos el componente QuoteBox para mostrar la cita actual y el botón de cambio */}
        <footer className="lg:flex lg:justify-center"> {/* Definimos el pie de página de la aplicación */}
          <h3 className="text-center text-lg text-black">Fuente: {quote.author}</h3> {/* Mostramos el autor de la cita actual */}
        </footer>
      </section>
    </main>
  );
